...
EdgeClass* find_edge(Triangulation* manifold, int edge_index)
{
    EdgeClass   *edge;
    ...
    for (edge = manifold->edge_list_begin.next;
	 ...
	 edge = edge->next)
    {
        if (edge->index == edge_index) return edge;
    }

    return NULL;
}

VertexIndex edge_base(Tetrahedron* tet, EdgeIndex e)
{
  return edge_direction(tet, e) ? one_vertex_at_edge[e] 
    : other_vertex_at_edge[e]; 
}

VertexIndex edge_tip(Tetrahedron* tet, EdgeIndex e)
{
  return edge_direction(tet, e) ? other_vertex_at_edge[e] 
    : one_vertex_at_edge[e]; 
}

void set_edge_direction(Tetrahedron* tet, EdgeIndex e, int d)
{
  if (d) tet->edge_direction |= (1 << e); 
  else tet->edge_direction &= (0x3f - (1 << e)); 
}

void invert_edge_direction(Tetrahedron* tet, EdgeIndex e)
{
  set_edge_direction(tet, e, 1 - edge_direction(tet,e));
}

int gluing_reverses_edge(Permutation gluing, EdgeIndex e)
{
  return EVALUATE(gluing,one_vertex_at_edge[e]) > EVALUATE(gluing,other_vertex_at_edge[e]);
}

int gluing_reverses_edge(EdgeIterator const& i)
{
  return gluing_reverses_edge(i.front_gluing(), i.edge_index());
}

void setup_edge_direction(EdgeClass* edge)
{
  EdgeIterator i(edge);
  for (; i.index < edge->order; ++i)
    set_edge_direction(i.tet, i.edge_index(), i.V[0] < i.V[1]); 
}

void invert_edge_direction(EdgeClass* edge)
{
  EdgeIterator i(edge);
  for (; i.index < edge->order; ++i)
    invert_edge_direction(i.tet, i.edge_index()); 
}

void direct_edges(Triangulation* manifold)
{
  EdgeClass *edge;
  EdgeIterator i;
...
/* 
 *  Code implementing the EdgeIterator class. 
 */ 

EdgeIterator::EdgeIterator(const EdgeClass* edge, int end)
  : tet(edge->incident_tet), index(0)
{
  int ind = edge->incident_edge_index;
  bool rh = tet->edge_orientation[ind] == right_handed;

  if (end==1) rh = !rh; 

  V[  end] = edge_base(tet, ind);
  V[1-end] = edge_tip (tet, ind);
  V[rh?2:3] = remaining_face[V[1]][V[0]];
  V[rh?3:2] = remaining_face[V[0]][V[1]]; 

  // if rh, base==0, and tip==1, then V[2]==2 and V[3]==3. 
}

EdgeIterator& EdgeIterator::operator ++()
{
  Permutation perm = front_gluing(); 
  tet  = tet->neighbor[V[3]];

  V[0] = EVALUATE(perm, V[0]); 
  V[1] = EVALUATE(perm, V[1]); 
  int tmp = EVALUATE(perm, V[2]); 
  V[2] = EVALUATE(perm, V[3]); 
  V[3] = tmp; 
  ++index; 
  return *this; 
}

EdgeIterator EdgeIterator::partner() const
{
  Permutation perm = back_gluing(); 
  EdgeIterator p; 
  p.tet = tet->neighbor[V[2]];
  int i; 
  for (i=0; i<4; i++) 
    p.V[i] = EVALUATE(perm, V[i]); 
  p.index= -index; 
  return p; 
}

void EdgeIterator::set_edge_base(EdgeClass* edge) const
{
  edge->incident_tet = tet; 
  edge->incident_edge_index = edge_index(); 
}

bool operator == (EdgeIterator const& a, EdgeIterator const& b)
{
  return a.tet == b.tet &&
    a.V[0] == b.V[0] && 
    a.V[1] == b.V[1] &&
...
EdgeIndex EdgeIterator::edge3() const 
{ 
  return edge3_between_vertices[V[0]][V[1]]; 
}

EdgeIndex EdgeIterator::edge_index() const 
{ 
  return edge_between_vertices[V[0]][V[1]]; 
}
