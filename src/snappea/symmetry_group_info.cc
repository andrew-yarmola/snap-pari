 ...
 *
 *  void get_abelianization(    SymmetryGroup   *symmetry_group,
 *                              SymmetryGroup   **commutator_subgroup,
 *                              SymmetryGroup   **abelianization);
 *
 *      Computes the commutator subgroup [G,G] and the abelianization
...
typedef struct CyclicWord2
{
    /*
     *  itsFactors points to a circular singly-linked list of Factors.
     *  The UI may decide whether to print a CyclicWord2 left-to-right or
    ...
    /*
     *  The size of a CyclicWord2 is the sum of the absolute values of the
     ...
     *  a NULL-terminated, singly-linked list of CyclicWord2s.
     */
    struct CyclicWord2  *next;
...
static Boolean          *compute_commutator_subgroup(SymmetryGroup *symmetry_group);
...
static void             combine_like_factors(CyclicWord2 *word);
static void             normalize_powers(CyclicWord2 *word, int *powers);
static void             normalize_power(int *power, int modulus);
static Boolean          remove_zero_factors(CyclicWord2 *word);
static CyclicWord2      *invert_cyclic_word(CyclicWord2 *word);
static Boolean          cyclic_word_is_on_list(CyclicWord2 *word, CyclicWord2 *list);
static Boolean          same_cyclic_word(CyclicWord2 *word0, CyclicWord2 *word1);
static Boolean          same_based_cyclic_word(Factor *word0, Factor *word1);
static void             compute_word_info(CyclicWord2 *word);
static Boolean          substitute_to_simplify(CyclicWord2 *helper, CyclicWord2 *target, int *powers);
static Boolean          substitute_word_to_simplify(CyclicWord2 *helper, CyclicWord2 *target, int *powers);
static int              cancellation_size(CyclicWord2 *word0, CyclicWord2 *word1, int *powers);
static void             insert_word(CyclicWord2 *helper, CyclicWord2 *target, int *powers);
static void             invert_relations_as_necessary(CyclicWord2 **relation_list);
static void             free_cyclic_word(CyclicWord2 *word);
    ...
    MatrixInt22 a_matrix)
{
    int i,
	j;
    static MatrixInt22  inverting_matrix = {{-1, 0}, {0, -1}};
    ...
    SymmetryGroup   **abelianization)
{
    ...
    {
	subset                  = compute_commutator_subgroup(symmetry_group);
	*commutator_subgroup    = create_subgroup(symmetry_group, subset);
	*abelianization         = create_quotient(symmetry_group, subset);
	my_free(subset);
    }
    else
    {
	*commutator_subgroup    = NULL;
	*abelianization         = NULL;
    }
}


static Boolean *compute_commutator_subgroup(
    SymmetryGroup   *symmetry_group)
{
    Boolean *subset,
	    progress;
    ...
    SymmetryGroupPresentation   *group)
{
    ...
    Factor      *temp1,
		...
		*relation,
		*last;
    CyclicWord2 **end_of_relation_list,
	...
	for (j = 0; j < symmetry_group->order; j++)
	{
		...
		/*
		 *  Create a new CyclicWord2, and install the
		...
		/*
		 *  Simplify the CyclicWord2 by combining adjacent Factors
		...
		/*
		 *  Compute the size of the CyclicWord2, the sum of its
		...
		/*
		 *  Install the new CyclicWord2 on itsRelations list
    ...
    CyclicWord2 *word)
{
	...
	do
	{
		 ...
		 *  list, eliminite it and leave the CyclicWord2 empty.
		 */
...
static CyclicWord2 *invert_cyclic_word(
    CyclicWord2 *word)
{
    CyclicWord2 *inverse_word;
    Factor      *factor,
		*inverse_factor;
    
    inverse_word                = NEW_STRUCT(CyclicWord2);
