...
/* Added by oag since include files do not seem to define it. */
#ifndef RAND_MAX 
#define RAND_MAX 0xffff
#endif

#include <vector>
#include <cstdio> /* added by oag for reporting in bump_cusps */ 
...
/* Set all the displacements. Don't do any error checking, assume
   user knows what she's doing. Added by oag. */ 

void displace_cusp_neighborhoods(
    CuspNeighborhoods   *cusp_neighborhoods,
    double              displacements[])
{
  Cusp *cusp;

  int c, nc = get_num_cusps(cusp_neighborhoods->its_triangulation); 

  for (c=0; c<nc; c++) {
    cusp = find_cusp(cusp_neighborhoods->its_triangulation, c);

    cusp->displacement += displacements[c];
    cusp->displacement_exp  = exp(cusp->displacement);
  }
...
void bump_cusps(CuspNeighborhoods *cn, int report)
{
  int i, nc = get_num_cusp_neighborhoods(cn);
  Triangulation* manifold = cn->its_triangulation; 
  vector<int> bumped(nc,0);
  EdgeClass *edge; 
  bool first_printed; 
  int c[2]; 
  double min_dist, this_dist; 
  double* displacements = new double[nc];

  while (true) {

    compute_intercusp_distances(manifold); 
    first_printed = false; 

    // check which cusp neighborhoods have already bumped. 
    for (edge = manifold->edge_list_begin.next;
	 edge != &manifold->edge_list_end;
	 edge = edge->next) {

      if (edge->intercusp_distance < INTERCUSP_EPSILON) {
	c[0] = edge->incident_tet->cusp[  one_vertex_at_edge[edge->incident_edge_index]]->index;
	c[1] = edge->incident_tet->cusp[other_vertex_at_edge[edge->incident_edge_index]]->index;

	if (report) {
	  for (i=0; i<2; i++) {
	    if (bumped[c[i]]) continue; 
	    bumped[c[i]] = true; 
	    if (!first_printed) { 
	      printf("%d",c[i]); first_printed=true; 
	    } else {
	      printf(",%d",c[i]); 
	    }
	  }
    ...
    // done if all have bumped. 
    for (i=0; i<nc; i++) 
      if (!bumped[i]) break; 
    if (i==nc) break; 

    if (report) { // there will be more bumpings so.. 
      if (first_printed) printf("|"); 
    }

    // see how much further we can advance the un-bumped neighborhoods. 
    min_dist = 1e12; 
    for (edge = manifold->edge_list_begin.next;
	 edge != &manifold->edge_list_end;
	 edge = edge->next) {
      ...
      c[0] = edge->incident_tet->cusp[  one_vertex_at_edge[edge->incident_edge_index]]->index;
      c[1] = edge->incident_tet->cusp[other_vertex_at_edge[edge->incident_edge_index]]->index;
      if (bumped[c[0]] && bumped[c[1]]) continue; 

      this_dist = (bumped[c[0]] || bumped[c[1]]) ? 
	edge->intercusp_distance : edge->intercusp_distance/2.0;

      if (this_dist < min_dist) min_dist = this_dist; 
    }

    // now move all the un-bumped cusps up. 
    for (i=0; i<nc; i++)
      displacements[i] = (bumped[i]) ? 0.0 : min_dist; 

    displace_cusp_neighborhoods(cn, displacements); 
  }
