    ...
    FillingStatus   which_structure)
{
	 ...
	 */
#ifndef NOPARI    
	compute_translation(&initial_ptet, i, direction[i], translation[i], which_structure, pari_trans[i]);
#else
	...
	if (complex_nonzero(translation[M][i]))
	    shape[i] = complex_div(translation[L][i], translation[M][i]);
	else
	    shape[i] = Infinity;

#ifndef NOPARI  
    if (pari_trans[M].type()!=1)
      cusp->acc_cusp_shape = pari_trans[L]/pari_trans[M];
#endif
...
#ifndef NOPARI
	cusp->acc_cusp_shape = - cusp->acc_cusp_shape;
#endif    
    }
    ...
    /* we don't conjugate the accurate/exact cusp shape in case it is 
       a polymod which we don't know how to take the conjugate of */
}

#ifndef NOPARI
static pari edge_param(pari shape, int face1, int face2)
{
  switch (edge3_between_faces[face1][face2]) {
  case 0:
    return shape; 
    break;
  case 1:
...
#ifndef NOPARI  
    do_pari_calc = (ptet.tet->acc_shape.type()!=1); 
    if (do_pari_calc) {
      pari_left_end = pZERO; 
      pari_right_end = pONE; 
    }
...
#ifndef NOPARI        
	    if (do_pari_calc) {
	      pari_old_diff = pari_right_end - pari_left_end;
	      pari_rotation = edge_param(ptet.tet->acc_shape,ptet.near_face,ptet.left_face);
	      if (ptet.orientation == left_handed) {
		if (pari_rotation.type()==9)
		  do_pari_calc = 0; 
		else
		  pari_rotation = pONE/gconj(pari_rotation); 
	      }
	      pari_new_diff = pari_old_diff * pari_rotation; 
	      pari_right_end = pari_left_end + pari_new_diff;
	    }
#endif        
...
#ifndef NOPARI        
	    if (do_pari_calc) {
	      pari_old_diff = pari_left_end - pari_right_end; 
	      pari_rotation = edge_param(ptet.tet->acc_shape,ptet.near_face,ptet.right_face);
	      if (ptet.orientation == left_handed) {
		if (pari_rotation.type()==9) 
		  do_pari_calc = 0; 
		else
	      ...
	      pari_new_diff = pari_old_diff / pari_rotation; 
	      pari_left_end = pari_right_end + pari_new_diff;
	    }
#endif        
