...
static ComplexWithLog regular_shape(Complex(0.5, ROOT_3_OVER_2),
		    Complex(0.0, PI_OVER_3));
...
static void free_cusp_status_arrays(
    ...
    double          *l_array)
{
    my_free_array(is_complete_array);
    my_free_array(m_array);
    my_free_array(l_array);
}
...
SolutionType do_Dehn_filling(Triangulation* manifold)
{
    return solve_for_holonomies(manifold, NULL, NULL, NULL); 
}
...
SolutionType solve_for_holonomies(
    Triangulation *manifold, 
...
*/
...
#if 0
    // For debugging, want to see holonomies & angle sums 
    // before we make any change to them. 
    compute_edge_angle_sums(manifold);
    print_holonomies(manifold); 
    print_edge_angle_sums(manifold); 
#endif
	...
	compute_gluing_equations(manifold, holonomies, h_is_mer);
    ...
    /* If holonomy_tangent is not zero it points to an array of 2*num_cusps
     * Complex numbers. The following code fills these in to give a set of 
     * vectors spanning the tangent space to the space of holonomies. Each vector
     * is a pair of successive entries representing a meridian and a longitude. 
     */ 
    if (holonomy_tangent!=NULL && manifold->orientability == oriented_manifold) {
      int i, n_cusps = get_num_cusps(manifold);
      Boolean *him = new Boolean[n_cusps];
      for (i=0; i<n_cusps; i++) him[i] = h_is_mer[i]; 
      for (i=0; i<n_cusps; i++) {
	him[i] = TRUE; 
	compute_gluing_equations(manifold, holonomies, him); 
	holonomy_tangent[2*i] = complex_determinant(complex_equations, num_rows, num_columns);
	him[i] = FALSE; 
	compute_gluing_equations(manifold, holonomies, him); 
	holonomy_tangent[2*i+1]=complex_determinant(complex_equations, num_rows, num_columns);
	him[i] = h_is_mer[i]; 
      }
      delete [] him; 
    }
    ...
    /* If we found a solution for specified holonomies, fill in the
     * Dehn filling coefficients appropriately. 
     */ 

    if (solution_was_found && h_is_mer != NULL) {
        ...
        for (cusp = manifold->cusp_list_begin.next;
	     ...
	     cusp = cusp->next)
	{
            Complex hm = cusp->holonomy[ultimate][M];
            Complex hl = cusp->holonomy[ultimate][L];
	    double det = hm.real * hl.imag - hm.imag * hl.real; 
	    if (fabs(det) < 1e-8) {
	        cusp->m = 0.1; // Junk values, can't be zero since CS code gets confused. 
		cusp->l = 0.1;
	    } else {
	        cusp->m = -TWO_PI * hl.real/det; 
		cusp->l =  TWO_PI * hm.real/det; 
	    }
    ...
    int     num_rows)
{
    ...
    for (i = 0; i < num_rows; i++)
	my_free_array(complex_equations[i]);

    my_free_array(complex_equations);
}
    ...
    int     num_rows)
{
    ...
    for (i = 0; i < num_rows; i++)
	my_free_array(real_equations[i]);

    my_free_array(real_equations);
}
...
static void free_arrays(
    ...
    CuspInfo    *save_cusp_info)
{
    my_free_array(save_shapes);
    my_free_array(save_cusp_info);
}
