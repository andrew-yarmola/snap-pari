...
static FGWord triangle_location_word(Tetrahedron* tet, int vertex)
{
  FGWord word;
  Extra* triangle = &tet->extra[vertex];
  int face, gen_index;
  GeneratorStatus gen_status;

  while (triangle->parent_tet) {
    
    /* What face pairing (if any) is involved? */ 
    face = triangle->this_faces_parent;
    gen_status = tet->generator_status[face];
    if (gen_status == inbound_generator || gen_status == outbound_generator) {
      gen_index = tet->generator_index[face] + 1; 
      if (gen_status == inbound_generator) gen_index = -gen_index; 
      word *= gen_index;
    } 
    
    tet = triangle->parent_tet;
    vertex = triangle->parent_vertex;
    triangle = &tet->extra[vertex]; 
  }
  return word;
}

// We assume that the fundamental group of this manifold has been computed, and 
// therefore that cusp->basepoint_tet and cusp->basepoint_vertex have already 
// been set up. We want to use these rather than just any old triangle in the cusp. 

bool get_end_pairing_words(Triangulation* manifold, vector<FGWord>& words)
{
  attach_extra(manifold);
  initialize_flags(manifold); 
  ...
  if (cusp->next != &manifold->cusp_list_end) return false; 

  PerimeterPiece *perimeter_anchor;
  set_up_perimeter(cusp->basepoint_tet, cusp->basepoint_vertex, &perimeter_anchor);
  expand_perimeter(perimeter_anchor);

  // Do we really need these? Let's assume not. 
  // find_mates(perimeter_anchor);
  // simplify_perimeter(&perimeter_anchor);

  // For each edge, find a pair of triangles. Record the words which
  // map the base_vertex to each of the two triangles. Composing one
  // way gives a word which maps one triangle onto the
  // other. Conjugating so that the first triangle is put where the
  // base triangle is, is equivalent to composing the other way. 
  ...
  words.resize(manifold->num_tetrahedra); 
  ...
  for (edge = manifold->edge_list_begin.next;
    ...
    a = triangle_location_word(tet, one_vertex_at_edge[edge->incident_edge_index]); 
    b = triangle_location_word(tet, other_vertex_at_edge[edge->incident_edge_index]); 

    if (i>=words.size()) break; // Should never happen. 
...
static bool check_vertex_sums(Tetrahedron* tet)
{
  int c, h, v, f, sum; 

  for (c=0; c<2; c++) { // c = meridian/longitude
    for (h=0; h<2; h++) { // h = handedness
      for (v=0; v<4; v++) { // v = vertex
	sum = 0; 
	for (f=0; f<4; f++) { // f = face
	  if (f==v) continue; 
	  sum += tet->curve[c][h][v][f];
	}
	if (sum) return false; 
      }
    }
...
static bool check_glued_sums(Tetrahedron* tet, FaceIndex f)
{
  int c, h, v, pv, pf, sum; 
  Tetrahedron *ptet = tet->neighbor[f]; 
  Permutation perm  = tet->gluing  [f]; 

  // want rh = 0 if orientation preserving. 
  int rh = (parity[perm]==orientation_reversing);
  pf = EVALUATE(perm, f); 
  ...
  for (c=0; c<2; c++) { // c = meridian/longitude
	...
	if (v==f) continue; 
	pv = EVALUATE(perm, v); 
	sum = tet->curve[c][h][v][f] + ptet->curve[c][(h+rh)%2][pv][pf];
	if (sum) return false; 
      }
    }
  }
...
bool check_peripheral_curves(Triangulation* manifold)
{
  Tetrahedron* tet; 
  FaceIndex f; 
  ...
  for (tet = manifold->tet_list_begin.next;
       tet != &manifold->tet_list_end;
       tet = tet->next) {
    if (!check_vertex_sums(tet)) return false; 
    for (f=0; f<4; f++) {
      if (!check_glued_sums(tet,f)) return false; 
    }
  }
  return true; 
}
