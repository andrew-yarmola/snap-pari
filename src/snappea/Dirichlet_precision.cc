...
static Boolean      precise_trace(const O31_matrix& m);
...
void MatrixPair::precise_matrix_pair()
{
    precise_matrix(m[0]);
    m[1] = inverse(m[0]); 
    _height = m[0].height(); 
    ...
    MatrixPairList  *gen_list)
{
    ...
    {
	matrix_pair->precise_matrix_pair(); 
    }
}
...
#else // NOPARI undefined. 

// When using Pari, O31 matrices should be very accurate 
// anyway making the code in this file redundant. 
