...
{
  ComplexWithLog() {}
  ComplexWithLog(const Complex& r, const Complex& l) : rect(r), log(l) {}
...
struct TetShape
{
    ...
    TetShape() {}
    TetShape(const TetShape& t) 
      { /* cwl = t.cwl; */
	copy_array((const ComplexWithLog *)t.cwl, (ComplexWithLog *)cwl, 6); }
    TetShape(const ComplexWithLog (&a)[2][3]) 
      { /* cwl = a; */ 
	copy_array((const ComplexWithLog *)a, (ComplexWithLog *)cwl, 6); }
    TetShape& operator = (const TetShape& t) 
      { /* cwl = t.cwl; */
	copy_array((const ComplexWithLog *)t.cwl, (ComplexWithLog *)cwl, 6);
	return *this; }
    ~TetShape() {}
...
struct Tetrahedron
{
    ...
    int                 edge_direction;     /* edge_classes.c -added by oag.            */
...
#ifndef NOPARI
    pari                acc_shape;          /* snap.cc & choose_generators.c (local)    */
    pari                acc_corners;        /* choose_generators.c (local)              */
#endif
...
struct Triangulation
{
    ...
    Boolean             proto_canonized;        /* canonize_part_1.c                    */
...
/* 
 *  EdgeIterator keeps track of a face at an EdgeClass and a direction 
 *  around that EdgeClass. It returns to its starting value after
 *  edge->order increments. 
 */

struct EdgeIterator {
  Tetrahedron* tet; 
  int V[4]; // V[0]=bottom, V[1]=top, V[2]=next, V[3]=this. 
  int index; 

  EdgeIterator() : tet(0), index(0) {}
  EdgeIterator(const EdgeClass* e, int end=0); 
  
  // default copy and assignment are ok. 

  FaceIndex face()  const { return V[2]; }
  FaceIndex front_face() const { return V[3]; }
  Permutation front_gluing() const { return tet->gluing[V[3]]; }
  Permutation back_gluing()  const { return tet->gluing[V[2]]; }

  EdgeIterator& operator ++(); 
  EdgeIterator  operator ++(int)
  { EdgeIterator i(*this); ++(*this); return i; }

  EdgeIterator partner() const;
  EdgeIndex edge3() const;
  EdgeIndex edge_index() const;

  void set_edge_base(EdgeClass* e) const; 
  
  friend bool operator == (EdgeIterator const& a, EdgeIterator const& b); 
  friend bool operator != (EdgeIterator const& a, EdgeIterator const& b)
    { return !(a==b); }
};
