 ...
 *  so their values shouldn't be changed.  (They are also toggled
 *  in Dirichlet_extras.c with the "not" operator '!'.)
 */
...
struct WEPolyhedron
{
    ...
    /*
     *  conjugacy is the matrix by which the matrix generators for the domain
     *  were conjugated in order to maximize injectivity radius. 
     *  Ie. group_element in each face is c^-1.m.c where c is conjugacy
     *  and m is the transformation given by word. Conjugacy maps from 
     *  Dirichlet domain coordinates to fundamental group coordinates. 
     */ 
...
class Dirichlet_stats {
public:
  double shortest_edge; 
  double plane_distance;
  double vertex_error; 
  double sphere_to_finite; 
  double ideal_vertex_error; 

  void clear();
  void print(); 
};

extern void print_Dirichlet_stats(); 
extern Dirichlet_stats stats; 
extern void track_error(double error, double& max_error);
extern void track_min(double value, double& min_value);
