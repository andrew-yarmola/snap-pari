...
static void             add_neighbors_to_s1(O31_matrix const& m, WEPolyhedron *polyhedron, IsometryListNode *s0, IsometryListNode *s1_begin, IsometryListNode ***s1_end);
static Boolean          intersection_is_nontrivial(WEPolyhedron *polyhedron, O31_matrix const& m);
static Boolean          vertex_in_polyhedron(O31_vector const& v, WEPolyhedron *polyhedron);
static void             add_isometry_list_node(IsometryListNode **isometry_list, O31_matrix const& m);
static void             add_isometry_tree_node(IsometryTreeNode **isometry_tree, O31_matrix const& m);
static Boolean          matrix_on_list(O31_matrix const& m, IsometryListNode *isometry_list);
static Boolean          matrix_on_tree(O31_matrix const& m, IsometryTreeNode *isometry_tree);
static double           key_value(O31_matrix const& m);
static void             add_shingle_node(ShingleNode **shingle_list, O31_matrix const& m0, O31_matrix const& m1, int index);
    ...
    int             num_layers)
{
    ...
    this_layer = NULL;
    next_layer = NULL;
    add_isometry_tree_node(&this_layer, ID);
    ...
    double      scale)
{
    ...
    static O31_vector   e1(0.0, 1.0, 0.0, 0.0),
			e2(0.0, 0.0, 1.0, 0.0);

    /*
     *  Let n be the current position of the shingle's normal vector.
     */
    n = position * shingle->normal;
    ...
    IsometryListNode    ***s1_end)
{
	...
	if (!close(mh, ID, SAME_MATRIX_EPSILON)
    ...
    IsometryListNode    *isometry_list)
{
	...
	if (close(node->m, m, SAME_MATRIX_EPSILON))
	    return TRUE;
    ...
    IsometryTreeNode    *isometry_tree)
{
	...
	if (left_flag && right_flag)
	    if (close(m, subtree->m, SAME_MATRIX_EPSILON))
		return TRUE;
    }
    ...
    int         index)
{
    ...
    static O31_vector zero_vector(0.0, 0.0, 0.0, 0.0);
     ...
     *  the shingling.
     */
    new_node->shingle.center  = zero_vector;
    new_node->shingle.radialA = zero_vector;
    new_node->shingle.radialB = zero_vector;
