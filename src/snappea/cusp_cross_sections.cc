...
/*
 * Compute tilt matrix whose inner product with the cusp 
 * displacement_exp's gives the tilts. Result is a matrix of 
 * nt by 4 by nc doubles, where nt is the number of tetrahedra and 
 * nc is the number of cusps. 
 */

double* compute_tilt_matrix(
    Triangulation *manifold)
{
    double      factor,
		R[4];
    ...
    /* tilt_mx is indexed by [tet][face][cusp] according to 
       the calculation [(tet*4 + face)*nc + cusp].
    */

    nt = manifold->num_tetrahedra; 
    nc = manifold->num_cusps;
    n = nt*4*nc; 
    tilt_mx = NEW_ARRAY(n, double); 
    for (i=0; i<n; i++) tilt_mx[i]=0.;
     ...
     */
	    ...
	    for (j = 0; j < 4; j++) {
  	        ...
  	        c = tet->cusp[j]->index;
	        if (c<0 || c>=nc) 
	          uFatalError("compute_tilt_matrix", "cusp_cross_sections"); 

	        if (j == i)

		    tilt_mx[(t*4 + i)*nc + c] -= R[j];
...
double* face_tilt_matrix(
    Triangulation *manifold)
{
    Tetrahedron *tet; 
    int t, ind, pind, i, pi, fi, nc, nt, c; 

    allocate_cross_sections(manifold); 
    compute_cross_sections(manifold); 

    /* index the tetrahedra */
    t=0; 
    for (tet = manifold->tet_list_begin.next;
	 tet != &manifold->tet_list_end;
	 tet = tet->next, t++) tet->index = t; 

    double* tilt_mx = compute_tilt_matrix(manifold); 

    nc = manifold->num_cusps; 
    nt = manifold->num_tetrahedra; 

    double* face_tilt_mx = NEW_ARRAY(2*nt * nc, double);
	...
	/* get the partner for this face */
	pind = tet->neighbor[i]->index;
	pi = EVALUATE(tet->gluing[i], i);

	/* do each face pair once only */ 
	if (pind < ind || (pind==ind && pi < i)) continue; 

	/* add together the tilt vectors for the paired faces */
	for (c=0; c<nc; c++) {
	  face_tilt_mx[fi*nc + c] = 
	    tilt_mx[(ind*4 + i)*nc + c] + tilt_mx[(pind*4 + pi)*nc + c];
	}
