    ...
    Tetrahedron *short_list_end)
{
	...
	for (i = 0; i < 4; i++)
	{
	    if (i == tet->extra->ideal_vertex_index)
		continue;

	    if (cancel_triangles(tet, i) == TRUE)
	    {
    ...
    FaceIndex   f0)
{
    ...
    REMOVE_NODE(tet);
    REMOVE_NODE(nbr_tet);
    if (tet->extra) my_free(tet->extra); 
    tet->extra = NULL; 
    if (nbr_tet->extra) my_free(nbr_tet->extra); 
    nbr_tet->extra = NULL; 
    ...
    Tetrahedron     *short_list_end)
{
	...
	for (i = 0; i < 4; i++)
	{
	    ...
	    if (two_to_two(manifold, tet, i, TRUE) == TRUE)
		return TRUE;
	}
    ...
    Tetrahedron     *tet)
{
	...
	if (tet->extra->Dehn_filling_curve[i] == 0)
	{
	    (void) two_to_two(manifold, tet, i, FALSE);
	    break;
	}
    }
}
    ...
    Tetrahedron     *tet0)
{
    ...
    {
	REMOVE_NODE(tet[i]);
	if (tet[i]->extra) my_free(tet[i]->extra); 
	tet[i]->extra = NULL; 
