 ...
 *  the corners are computed using the hyperbolic structure of the Dehn
 *  filled manifold.  If centroid_at_origin is TRUE, the initial tetrahedron
 *  is positioned with its centroid at the origin;  otherwise the initial
 *  tetrahedron is positioned with its vertices at {0, 1, infinity, z}.
 */
...
static void initial_corners(Tetrahedron *tet, Boolean centroid_at_origin);
...
#ifndef NOPARI
#include "pariwrap.hh"
static pari edge_param(pari shape, int edge3);
#endif
    ...
    Boolean         centroid_at_origin)
{
    ...
    /*
     *  Put an arbitrary Tetrahedron on the queue,
     *  and mark it as visited.
     */
    queue_first = 0;
    queue_transp= 0;
    queue_last  = 0;
    queue[0] = manifold->tet_list_begin.next;
    queue[0]->generator_path = -1;
    queue[0]->flag = right_handed;
    if (compute_corners)
	initial_corners(queue[0], centroid_at_origin);
    ...
    while (1) {
        ...
        /*  Find an unattached face to work on. */
        if (manifold->proto_canonized) {

	  /* Look for a transparent face first. */
	  for (;queue_transp <= queue_last; queue_transp++) {
	    tet = queue[queue_transp];
	    for (face=0; face<4; face++) { 
	      if (tet->generator_status[face]!=unassigned_generator) continue;
	      ...
	      if (sum_of_tilts >= -CONCAVITY_EPSILON) break; /* Found one. */
	    }
	    if (face<4) break; 
	  }
	  if (queue_transp <= queue_last) found_face = TRUE;
	}

	if (!found_face) {

	  /* Find any face, transparent or otherwise. */
	  for (;queue_first <= queue_last; queue_first++) {
	    tet = queue[queue_first];
	    for (face=0; face<4; face++) { 
	      if (tet->generator_status[face]==unassigned_generator) break;
	    }
	    if (face<4) break; 
	  }
	...
	if (!found_face) break; /* We're finished. */
	...
	if (nbr_tet->flag == unknown_orientation) {
	  ...
	  // cout << nbr_tet << " " << int(nbr_face) << ' ' << tet << endl; 
	  ...
	  if (compute_corners) {
...
#ifndef NOPARI                    
	      if (tet->acc_corners.type()!=1) {
		/* initialize acc_corners of neighbor if necessary */
		if (nbr_tet->acc_corners.type()==1)
		  nbr_tet->acc_corners = rvector(4); 
		nbr_tet->acc_corners[nbr_i] = tet->acc_corners[i];
	      }
#endif                    
	    }
	    compute_fourth_corner(
	       ...
	       nbr_tet->acc_shape
#else
	       nbr_tet->shape[filled]->cwl[ultimate]  
#endif            
	       );
	  }
	  ...
	  /*
	   *  Since nbr_tet has been visited, assign a generator to the common face.
	   */
...
static pari edge_param(pari shape, int edge3)
{
  ...
  case 0:
    ...
    break;
  case 1:
    return pONE/(pONE - shape); 
    break;
  case 2:
    return (shape - pONE)/shape; 
    break;
  default:
    break; 
  }
    ...
    Boolean     centroid_at_origin)
{
	...
	z = tet->shape[filled]->cwl[ultimate][0].rect;
...
#ifndef NOPARI    
	if (tet->acc_shape.type()!=1) {

	  pari acc_z = tet->acc_shape;
	  pari acc_w = rvector(4); 

	  acc_w[0] = gsqrt(acc_z) + gsqrt(acc_z - pONE);
	  acc_w[1] = (pONE/acc_w[0]);
	  acc_w[2] = -(acc_w[1]);
	  acc_w[3] = -(acc_w[0]);

	  /* initialize acc_corners */ 
	  tet->acc_corners = rvector(4); 
	  tet->acc_corners[0] = acc_w[0];
	  tet->acc_corners[1] = acc_w[1];
	  tet->acc_corners[2] = acc_w[2];
	  tet->acc_corners[3] = acc_w[3];
	}
#endif    
    ...
    {
	/*
	 *  Position the Tetrahedron's vertices at {0, 1, infinity, z}.
	 *  Let the edge with the largest dihedral angle (in the complete
	 *  structure) pass through the basepoint.  Use the complete
	 *  structure rather than the filled structure for this decision
	 *  so that the resulting matrix generators vary continuously
	 *  during Dehn filling.  The final z value is, of course, that
	 *  of the filled structure.
	 *
	 *  Note:  In cases where four 45-45-90 ideal tetrahedra
	 *  meet to form an ideal octahedron, this puts the octahedron's
	 *  centroid at the origin, as desired.
	 */

	/*
	 *  Find the largest dihedral angle.
	 *  The dihedral angles sum to pi, so at least one will be positive.
	 */
	max_angle   = 0.0;
	max_angle_i = -1;
	for (i = 0; i < 3; i++)
	    if (max_angle < tet->shape[complete]->cwl[ultimate][i].log.imag)
	    {
		max_angle = tet->shape[complete]->cwl[ultimate][i].log.imag;
		max_angle_i = i;
	    }
	if (max_angle == -1)
	    uFatalError("initial_corners", "choose_generators.c");

	/*
	 *  Order the vertices so that the tetrahedron is positively
	 *  oriented, and the largest dihedral angle is between vertices
	 *  v[0] and v[1].
	 */
	v[0] = one_vertex_at_edge[max_angle_i];
	v[1] = other_vertex_at_edge[max_angle_i];
	...
	tet->corner[v[1]] = Zero;
	tet->corner[v[2]] = One;
	tet->corner[v[3]] = tet->shape[filled]->cwl[ultimate][max_angle_i].rect;
	...
	if (tet->acc_shape.type()!=1) {
	  ...
	  tet->acc_corners[v[0]] = pHUGE;
	  tet->acc_corners[v[1]] = pZERO;
	  tet->acc_corners[v[2]] = pONE;
	  tet->acc_corners[v[3]] = edge_param(tet->acc_shape, max_angle_i);
	}
#endif    
    }
}
...

{
...
#ifndef NOPARI  
    Boolean accurate = (acc_shape.type()!=1);
    pari acc_cross_ratio, acc_z;

    if (accurate) {
      acc_z = rvector(4); 
      for (i = 0; i < 3; i++)
	acc_z[i] = acc_corners[v[i]];

      acc_cross_ratio = edge_param(acc_shape, edge3_between_faces[v[0]][v[1]]);

      if (orientation == left_handed)
	acc_cross_ratio = gconj(pONE/acc_cross_ratio);
    }
#endif  
	...
	if (complex_nonzero(denominator) == TRUE)
	    z[3] = complex_div(numerator, denominator);
	else if (complex_nonzero(numerator) == TRUE)
	    z[3] = Infinity;
	else 
 	    z[3] = One;

#if 0 // removed by oag -- garbage in, garbage out but don't crash. 
	 else
	      uFatalError("compute_fourth_corner", "choose_generators");
#endif 
...
#ifndef NOPARI  
    if (accurate) {
      if (sizedigit(acc_z[0]) > digits)
	acc_z[3] = acc_z[1] + acc_cross_ratio * (acc_z[2] - acc_z[1]);
      else {
	pari acc_diff20 = acc_z[2] - acc_z[0];
	pari acc_diff21 = acc_z[2] - acc_z[1];

	pari acc_numerator = acc_z[1] * acc_diff20 - 
	  acc_cross_ratio * acc_z[0] * acc_diff21; 

	pari acc_denominator = acc_diff20 - acc_cross_ratio * acc_diff21;


	if (sizedigit(acc_denominator) > -digits)
	  acc_z[3] = (acc_numerator/acc_denominator);

	else if (sizedigit(acc_numerator) > -digits)
	  acc_z[3] = pHUGE;

	else
	  acc_z[3] = pONE; // garbage value. 
      }

      acc_corners[missing_corner] = acc_z[3];
