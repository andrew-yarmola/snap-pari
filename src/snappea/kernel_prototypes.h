...
#ifndef NOPARI
inline void compute_fourth_corner(
    Complex         corner[4],
    VertexIndex     missing_corner,
    Orientation     orientation,
    ComplexWithLog  cwl[3])
{
    compute_fourth_corner(corner, missing_corner, orientation, cwl, pZERO, pZERO);
}
#endif
...
extern EdgeClass *find_edge(Triangulation* manifold, int edge_index); 
/*
 *  Converts an edge_index into an EdgeClass pointer. 
 */ 

inline int edge_direction(Tetrahedron* tet, EdgeIndex e)
{ return (tet->edge_direction >> e) & 1; }
/*
 *  Return 1 (true) if edge direction goes from lower numbered vertex
 *  to higher numbered vertex in this tetrahedron. 
 */ 

VertexIndex edge_base(Tetrahedron* tet, EdgeIndex e);
VertexIndex edge_tip(Tetrahedron* tet, EdgeIndex e);
/*
 *  Return the vertex at base/tip respectively of edge e in tet. 
 */ 

extern void direct_edges(Triangulation* manifold);
/*
 *  Set all the edge_direction fields consistently (but arbitrairly). 
 */

extern void setup_edge_direction(EdgeClass* edge);
/* 
 *  Ensure that edge direction is consistent around this edge class. 
 */
...
extern bool check_peripheral_curves(Triangulation* manifold);
/*
 *  Make sure that peripheral curves sum to zero in each
 *  triangular vertex cross section and across each edge. 
 */ 
...
extern FuncResult uncancel_tetrahedra(EdgeClass *edge, int face[2], EdgeClass **new_e, int* num_tetrahedra_ptr);
...
extern Complex complex_determinant(Complex **complex_equations, 
...
extern void clear_triangulation(Triangulation *manifold); 
/* 
 * Frees everything hanging off a Triangulation structure.
 */ 

extern void set_triangulation(Triangulation *source, Triangulation *destination);
/*
 * Copies source to destination, which is assumed to be uninitialized. 
 */ 

extern Tetrahedron* find_tetrahedron(Triangulation *manifold, int index);
/*
 * Converts a Tetrahedron index into a pointer. 
 */ 
...
extern Boolean check_gluings(Triangulation* manifold);
/*
 *  Returns true if all gluing fields are set consistently. 
 */
