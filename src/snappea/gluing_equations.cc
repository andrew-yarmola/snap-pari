...
static void compute_derivative(Triangulation *manifold, const Boolean h_is_mer[]);
static void compute_rhs(Triangulation *manifold, const Complex holonomies[], const Boolean h_is_mer[]);

/* 
 * The second and third parameters of this equation allow us to solve
 * the gluing equations for specified holonomies. We can either solve
 * for the meridian or longitude at each cusp. If both entries are
 * NULL we use the Dehn filling coefficients to determine the
 * equations to be solved. If supplied, each should point to an array
 * of length equal to the number of cusps of the manifold.
 */
    ...
    const Boolean   h_is_mer[])
{
    ...
    compute_derivative(manifold, h_is_mer);
    compute_rhs(manifold, holonomies, h_is_mer);
}
    ...
    const Boolean   h_is_mer[])
{
	...
	for (v = 0; v < 4; v++)     /* Look at each ideal vertex. */
	{
	    ...
	    else if (tet->cusp[v]->is_complete)
	    {
		m = 1.0;
		l = 0.0;
	    }
    ...
    const Boolean   h_is_mer[])
{
	 ...
	 */
        ...
        if (h_is_mer) {
	    desired_holonomy = holonomies[cusp->index]; 
	    current_holonomy = h_is_mer[cusp->index] ? 
	      cusp->holonomy[ultimate][M] : cusp->holonomy[ultimate][L];
	}
	else if (cusp->is_complete)
	{
