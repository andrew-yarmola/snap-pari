...
#include "length_spectrum.h"

static double       key_value(O31_matrix const& m);
static Boolean      already_on_tree(Tile *root, Tile *tile);
static Tile*        find_tile(Tile *root, Tile *tile);
static void         add_to_tree(Tile *root, Tile *tile);
static void         set_words(Tile *root, const WEPolyhedron *polyhedron);
...
static void         queue_peripheral_tiles(Tile *root, Tile *queue_end);

static O31_matrix ID(1); 

FGWord tile_word(Tile* t)
{
  return t->word; 
}

FGWord tile_word(Tile* root, Tile* t, const WEPolyhedron *polyhedron)
{
  /* Perhaps it has already been computed. */ 
  if (t==root || t->word.length()!=0) return t->word; 

  Tile nbr;
  Tile *nbr_ptr; 
  FGWord candidate; 
  WEFace *face;
  int closer_nbr_count = 0; 
  ...
  for (face = polyhedron->face_list_begin.next;
       ...
       face = face->next)
    {
      ...
      nbr.g = t->g * *face->group_element; 
      // o31_product(t->g, *face->group_element, nbr.g);

      /*
       *  If nbr is closer to the origin than t, we compare t's word
       *  with the word it receives from nbr and choose the shortlex least. 
       */
      if (nbr.g(0,0) < t->g(0,0) - ISOMETRY_EPSILON) {

	nbr.key = key_value(nbr.g);
	nbr_ptr = find_tile(root, &nbr); 

	if (!nbr_ptr) {
	  printf("There appears to be a hole in this tiling!!\n"); 
	  continue;
	}
	closer_nbr_count++; 

	candidate = tile_word(root, nbr_ptr, polyhedron) * inverse(face->word); 

	if (t->word.length()==0 || candidate < t->word)
	  t->word = candidate; 
      }
    }

  if (!closer_nbr_count) {
    printf("No neighbor of this tile is closer to the origin than it is.\n"); 
  }
...
{
  int lcmp = uhs_compare(a.length, b.length, DUPLICATE_LENGTH_EPSILON); 
  if (lcmp < 0) return true;
  if (lcmp > 0) return false; 
  return a.word < b.word; 
}

ostream& operator << (ostream& out, GeodesicWord const& gw)
{ 
  return out << gw.length << ' ' << gw.word; 
}
    ...
    Boolean         get_words)
{
     ...
     */
    int num_translates = count_translates(tiling);
    ...
    /*
     *  We want to retain precisely one
    ...
    eliminate_conjugates(geodesic_list,
			    ...
			    tiling,
			    num_translates,
			    spine_radius + CONJUGATE_SPINE_EPSILON);


    if (*geodesics) my_free_array(*geodesics);

    if (num_good_geodesics > 0) {

	*geodesics = NEW_ARRAY(num_good_geodesics, GeodesicWord);

	int i; 
	for (i = 0; i < num_good_geodesics; i++) {
	  (*geodesics)[i].length = geodesic_list[i]->length;
	  (*geodesics)[i].mt = MoebiusTransformation(geodesic_list[i]->g);
	  
	  if (get_words)
	    (*geodesics)[i].word = geodesic_list[i]->word;
	}
	*num_geodesics = num_good_geodesics;
    ...
    int             *num_lengths)
{
	...
	polyhedron,
	full_rigor ?
	    2 * acosh( cosh(polyhedron->spine_radius) * cosh(cutoff_length/2) ) :
	    user_radius,
	&tiling);
    ...
    Tile            **tiling)
{
    ...
    identity->key           = key_value(ID);
    identity->next_subtree  = NULL;
    identity->peripheral_tile = TRUE; 

    *tiling = identity;

    expand_tiling(polyhedron, tiling_radius, *tiling);
    ...
    Tile            *root)
{
    ...
    //    std::cerr << "tiling to radius " << tiling_radius << " ...";
    ...
    queue_peripheral_tiles(root, &queue_end);
	     ...
	     face = face->next)
	{
	     ...
	     *  Compute the neighbor's group element and key value.
	     */
	    nbr->g = tile->g * *face->group_element;
	    ...
	    if (nbr->g(0,0) < cosh_tiling_radius) {
	      ...
	      if (already_on_tree(root, nbr) == FALSE)
		{
		  nbr->parent       = tile; 
		  /* nbr->word         = face->word; */ 
		  nbr->length       = complex_length(nbr->g);
		  nbr->parity       = determinant(nbr->g) > 0.0 ?
		    orientation_preserving :
		      orientation_reversing;
		  nbr->topology = orbifold1_unknown;
		  add_to_tree(root, nbr);
		  INSERT_BEFORE(nbr, &queue_end);
		}
	      else
		my_free(nbr); /* The neighbor was already on the tree. */ 
	    }
	    else
	      {
		/*
		 *  The neighbor was beyond the tiling_radius.
		 */
		my_free(nbr);
		tile->peripheral_tile = TRUE; 
	      }
	}
    }

    set_words(root, polyhedron);
    ...
    O31_matrix const& m)
{
     ...
     *  about that.
     *
     *  Recall that the first column of an O31_matrix gives the image
     *  of the origin.
     */
    ...
    Tile    *tile)
{
	...
	if (left_flag && right_flag)
	    if (close(subtree->g, tile->g, ISOMETRY_EPSILON))
		return TRUE;
    }
    ...
    Tile    *tile)
{
    ...
    delta = tile->key - root->key;
    
    /*
     *  Check this Tile if the key values match.
     */
    if (fabs(delta) < TREE_EPSILON)
      if (close(root->g, tile->g, ISOMETRY_EPSILON))
	return root;

    if (delta < +TREE_EPSILON && root->left_child) {
      the_tile = find_tile(root->left_child, tile);
      if (the_tile) return the_tile; 
    }
    if (delta > -TREE_EPSILON && root->right_child) {
      the_tile = find_tile(root->right_child, tile);
      if (the_tile) return the_tile; 
    }
    ...
    Tile    *root)
{
    ...
    TilingIterator it;
    for (it = root; it; it++) {
	dist = distance_to_origin(it.tile()); 
...
/* 
 *  This is confusing but basically it does 
 *  a depth first search, always going to the right first. 
 *
 *  Since the iterator requires a stack of positions in
 *  the tree structure to keep track of its place, and 
 *  these are stored in the tree itself (for efficiency 
 *  I guess) there can only be ONE LIVE ITERATOR AT AT TIME!
 *  -oag.
 */

void TilingIterator::operator ++ ()
{
  if (!subtree_stack) return; 

  /*
   *  Pull a subtree off the stack.
   */
  Tile* subtree           = subtree_stack;
  subtree_stack           = subtree_stack->next_subtree;
  subtree->next_subtree   = 0;
    ...
    const WEPolyhedron *polyhedron)
{
  ...
  for (i=root; i; i++) {
    ...
    /* Set i->word. */ 
    tile_word(root, i.tile(), polyhedron); 
  }
}
...
static double distance_to_origin(Tile *tile)
{
  return distance_to_origin(tile->g, tile->length, 
			    tile->parity == orientation_reversing); 
}

double distance_to_origin(O31_matrix const& g, 
			  Complex length, 
			  bool or_rev)
{
    double  cosh_d,
    ...
    if (or_rev)
    {
        return distance_to_origin(g * g, Complex(2*length.real,0.), false);
    }
    ...
    cosh_d  = g(0,0);
    cosh_s  = cosh(length.real);
    cos_t   = cos(length.imag);
     ...
     */
    return acosh(safe_sqrt( (cosh_d - cos_t) / (cosh_s - cos_t) ));
}
    ...
    double  cutoff_length)  /*  2*LENGTH_EPSILON has already been added in  */
{
	...
	(
	    ...
	    if (geodesic_list[i]->length.real
		    > the_power.length.real - LENGTH_EPSILON)
	    {
		if (close(geodesic_list[i]->g, the_power.g, ISOMETRY_EPSILON) == TRUE)
		{
    ...
    double  spine_radius)
{
    ...
    double  distance, conjugate_distance; 
    Tile    the_conjugate,
	    the_inverse,
     ...
     *  conjugate to its own inverse, and thereby determine its topology.
     * 
     *  Added by oag: If this geodesic has a conjugate which is closer to 
     *  the origin, we mark this geodesic to_be_eliminated and return. The 
     *  the result should be a list in which only the conjugate closest to
     *  the origin is retained. 
     */

    distance = distance_to_origin(geodesic_list[i0]); 
     ...
     *  Set up the inverse.
     */
    the_inverse.g = inverse(geodesic_list[i0]->g);
     ...
     */
    conjugator_cutoff = cosh( 2 * acosh(
	 ...
	 */
	the_conjugate.g = geodesic_list[i0]->g;
	the_conjugate.g.conjugate_by(conjugator_list[j]->g);
	 ...
	 */
	conjugate_distance = distance_to_origin(&the_conjugate);
	if (conjugate_distance > spine_radius)
	    continue;

	/* 
	 *  If the conjugate is closer to the origin than this 
	 *  geodesic, we immediately lose interest in this geodesic. 
	 */ 
	if (conjugate_distance < distance - POSSIBLE_CONJUGATE_EPSILON) {
	    geodesic_list[i0]->to_be_eliminated = TRUE; 
	    break; 
	}
	 ...
	 */
	the_inverse_conjugate.g = the_inverse.g;
	the_inverse_conjugate.g.conjugate_by(conjugator_list[j]->g);
	 ...
	 */
	if (close(the_conjugate.g, the_inverse.g, ISOMETRY_EPSILON) == TRUE)
	...
	for (i = i0 + 1; i < num_good_geodesics; i++)
	{
	     ...
	     */
	    if (close(geodesic_list[i]->g, the_conjugate.g, ISOMETRY_EPSILON) ||
		close(geodesic_list[i]->g, the_inverse_conjugate.g, ISOMETRY_EPSILON)) {

		/* 
		 *  As before, if we find a conjugate which looks
		 *  better than geodesic_list[i0], mark the latter to
		 *  be eliminated and return. -oag 
		 */
		if (conjugate_distance < distance + POSSIBLE_CONJUGATE_EPSILON &&
		    geodesic_list[i]->word < geodesic_list[i0]->word) {
		    geodesic_list[i0]->to_be_eliminated = TRUE; 
		    return;
		}
    ...
    WEPolyhedron    *polyhedron)
{
	...
	if (determinant(*face->group_element) < 0.0)
    ...
    Boolean     manifold_is_orientable)
{
    ...
    *spectrum = NEW_ARRAY(*num_lengths, MultiLength);
     ...
     */
    my_free_array(multilength_array);
}
...
static void queue_peripheral_tiles(
    Tile    *root, Tile *queue_end)
{
    Tile    *subtree_stack,
	    *subtree;

    /*
     *  Implement the recursive algorithm using our own stack
     *  rather than the system stack, to avoid the possibility of a
     *  stack/heap collision.
     */
	 ...
	 *  Queue the tile. 
	 */
	if (subtree->peripheral_tile) {
	  INSERT_BEFORE(subtree, queue_end); 
	}
    }
}
