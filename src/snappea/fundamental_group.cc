...
struct GroupPresentation
{
     ...
     */
    O31_matrix   *itsMatrices;

    /* 
     * added by oag
     */
    MoebiusTransformation *itsMoebiusTrs; 
...
static GroupPresentation    *compute_unsimplified_presentation(Triangulation *manifold, Boolean normalized);
static void                 compute_matrix_generators(Triangulation *manifold, GroupPresentation *group, Boolean normalized);
...
static int                  *fg_get_cyclic_word(const CyclicWord *list, int which_relation);
static void                 free_word_list(CyclicWord *aWordList);

static O31_matrix ID(1); 

static MoebiusTransformation& MI()
{
  static MoebiusTransformation id(One); 
  return id; 
}
    ...
    Boolean         normalize_moebius_trs)
{
     ...
     */
    group = compute_unsimplified_presentation(manifold, normalize_moebius_trs);
    ...
    Boolean         normalized)
{
    ...
    compute_matrix_generators(manifold, group, normalized);
    ...
    Boolean             normalized)
{
    ...
    group->itsMatrices = NEW_ARRAY(manifold->num_generators, O31_matrix);
    group->itsMoebiusTrs = NEW_ARRAY(manifold->num_generators, MoebiusTransformation);
	...
	matrix_generators(manifold, group->itsMoebiusTrs, FALSE, normalized);

	Moebius_array_to_O31_array( group->itsMoebiusTrs,
				    group->itsMatrices,
	...
	for (i = 0; i < manifold->num_generators; i++) {
	    group->itsMatrices[i] = ID;
	    group->itsMoebiusTrs[i] = MI(); 
	}
    }
}
    ...
    GroupPresentation   *group)
{
    ...
    {
	if (a == 0)     /*  There is no generator   */
	    continue;

	for (b = - group->itsNumGenerators; b <= group->itsNumGenerators; b++)
	{
	    if (b == 0)     /*  There is no generator   */
		continue;
...
// Add a generator to the presentation. 
// Its matrix is set to be the identity. 
// The new number of generators is returned. 

static int add_generator(GroupPresentation* group)
{
    int i;
    O31_matrix *new_array;
    MoebiusTransformation *new_Moebius_array;

    /*
     *  Allocate a bigger array for the matrix generators,
     *  copy in the existing values, and free the old array.
     */
    new_array = NEW_ARRAY(group->itsNumGenerators + 1, O31_matrix);
    new_Moebius_array = NEW_ARRAY(group->itsNumGenerators + 1, 
				  MoebiusTransformation);
    for (i = 0; i < group->itsNumGenerators; i++) {
	new_array[i] = group->itsMatrices[i];
	Moebius_copy(&new_Moebius_array[i], &group->itsMoebiusTrs[i]);
      }
    my_free_array(group->itsMatrices);
    my_free_array(group->itsMoebiusTrs);
    group->itsMatrices = new_array;
    group->itsMoebiusTrs = new_Moebius_array;

    /*
     *  Create the new matrix.
     */

    group->itsMatrices[group->itsNumGenerators] = ID;
    group->itsMoebiusTrs[group->itsNumGenerators] = MI();
...
{
  add_generator(group);
  group->itsMoebiusTrs[group->itsNumGenerators-1] = m;
  group->itsMatrices[group->itsNumGenerators-1] = m;
  return group->itsNumGenerators;
}
    ...
    int                 length)
{
    ...
    O31_matrix the_inverse;
    MoebiusTransformation the_Moebius_inverse; 
    int         i;
    Letter      *letter,
    ...
    int newgen = group->itsNumGenerators - 1; 
	...
	if (letter->itsValue > 0) {
	    ...
	    group->itsMatrices[newgen] *= 
	      group->itsMatrices[letter->itsValue - 1];

	    Moebius_product(&group->itsMoebiusTrs[newgen],
			&group->itsMoebiusTrs[letter->itsValue - 1],
			&group->itsMoebiusTrs[newgen]);
	  }
	else
	{
	    the_inverse = inverse(group->itsMatrices[(-letter->itsValue) - 1]);
	    group->itsMatrices[newgen] *= the_inverse; 

	    Moebius_invert( &group->itsMoebiusTrs[(-letter->itsValue) - 1],
			&the_Moebius_inverse);
	    Moebius_product(&group->itsMoebiusTrs[newgen],
			&the_Moebius_inverse,
			&group->itsMoebiusTrs[newgen]);
...
{
  word->next = group->itsRelations;
  group->itsRelations = word;
  group->itsNumRelations++; 
}
    ...
    int                 a)
{
    ...
    group->itsMatrices[a - 1].invert(); 
    Moebius_invert(&group->itsMoebiusTrs[a - 1], &group->itsMoebiusTrs[a - 1]);
    ...
    int                 b)
{
    ...
    O31_matrix   temp;
    MoebiusTransformation Moebius_temp; 
	...
	if (b > 0)
	{
	     ...
	     *  Use M(a') = M(a) M(b).
	     */
	    group->itsMatrices[a-1] *= group->itsMatrices[b-1];

	    Moebius_product(&group->itsMoebiusTrs[a-1],
			    &group->itsMoebiusTrs[b-1],
			    &group->itsMoebiusTrs[a-1]);
	}
	else    /* b < 0 */
	{
	    /*
	     *  Use M(a') = M(a) [M(B)^-1].
	     */
	    group->itsMatrices[a-1] *= inverse(group->itsMatrices[(-b)-1]);

	    Moebius_invert( &group->itsMoebiusTrs[(-b)-1],
			    &Moebius_temp);
	    Moebius_product(&group->itsMoebiusTrs[a-1],
			    &Moebius_temp,
			    &group->itsMoebiusTrs[a-1]);
	...
	if (b > 0)
	{
	     ...
	     *  Use M(A') = [M(b)^-1] M(A)
	     */
	    group->itsMatrices[(-a)-1].left_mul(inverse(group->itsMatrices[b-1]));
	    Moebius_invert( &group->itsMoebiusTrs[b-1],
			    &Moebius_temp);
	    Moebius_product(&Moebius_temp,
			    &group->itsMoebiusTrs[(-a)-1],
			    &group->itsMoebiusTrs[(-a)-1]);
	}
	else    /* b < 0 */
	{
	     ...
	     *  Use M(A') = M(B) M(A)
	     */
	    group->itsMatrices[(-a)-1].left_mul(group->itsMatrices[(-b)-1]); 

	    Moebius_product(&group->itsMoebiusTrs[(-b)-1],
			    &group->itsMoebiusTrs[(-a)-1],
			    &group->itsMoebiusTrs[(-a)-1]);
	}
    }
    ...
    CyclicWord          *word)
{
    ...
    group->itsMatrices[dead_generator - 1] = 
      group->itsMatrices[group->itsNumGenerators - 1];
    Moebius_copy(&group->itsMoebiusTrs[dead_generator - 1],
		&group->itsMoebiusTrs[group->itsNumGenerators - 1]);
    ...
    MoebiusTransformation   *result_Moebius)
{
     ...
     *  computes the value of a word not only as an O31_matrix, but also
    ...
    *result_Moebius = MI(); 
	...
	{
 	    theO31Factor = group->itsMatrices[*word - 1];
	    Moebius_copy(&theMoebiusFactor, &group->itsMoebiusTrs[*word - 1]);
	}
	else
	if (*word < 0
	 && *word >= - group->itsNumGenerators)
	{
 	    theO31Factor = inverse(group->itsMatrices[-(*word) - 1]);
	    Moebius_invert(&group->itsMoebiusTrs[-(*word) - 1], &theMoebiusFactor);
	}
	else
	    uFatalError("fg_word_to_matrix", "fundamental_group");

	/*
	 *  ...and right-multiply it onto the matrix_generator.
	 */
	result_O31 *= theO31Factor;
...
static int *fg_get_cyclic_word(
    const CyclicWord  *list,
    int         which_relation)
{
    int         i;
    const CyclicWord  *word;
    const Letter      *letter;
    int         *result;
    ...
    int              which_relation)
{
	...
	if (word != NULL)
	    word = word->next;
    if (word == NULL)
	uFatalError("fg_cyclic_word", "fundamental_group");

    for (   i = 0, letter = word->itsLetters;
	    i < word->itsLength;
	    i++, letter = letter->next)
    {
	result *= letter->itsValue;
    }
...
{
    if (which_relation < 0 || which_relation >= group->itsNumRelations)
	uFatalError("fg_relation", "fundamental_group");

    return fg_cyclic_word(group->itsRelations, which_relation);
}
...
{
    if (which_cusp < 0 || which_cusp >= group->itsNumCusps)
	uFatalError("fg_meridian", "fundamental_group");

    return fg_cyclic_word(group->itsMeridians, which_cusp);
}
...
{
    if (which_cusp < 0 || which_cusp >= group->itsNumCusps)
	uFatalError("fg_longitude", "fundamental_group");

    return fg_cyclic_word(group->itsLongitudes, which_cusp);
}


FGWord fg_original_generator(
    const GroupPresentation *group,
    int                     which_generator)
{
    if (which_generator < 0 || which_generator >= group->itsNumOriginalGenerators)
	uFatalError("fg_original_generator", "fundamental_group");

    return fg_cyclic_word(group->itsOriginalGenerators, which_generator);
}

FGWord fg_word_from_original(
    const GroupPresentation *group,
    FGWord const& orig)
{
  FGWord w, gw; 
  int i, n = orig.length(), g; 
  for (i=0; i<n; ++i) {
    g = orig[i];
    gw = fg_original_generator(group, (g<0)?-g-1:g-1);
    w *= ((g<0)?inverse(gw):gw);
  }
  return w; 
}
...
// Added by oag. 
// I want to be able to modify the presentation by 
// adding extra generators and relations. Since the
// relations are available as FGWords I'll need to 
// convert them to CyclicWords. Ultimately it would
// be better to do it all in terms of one or the other 
// but for the time being we'll use a conversion. 

// We'll set is_Dehn_relation to false. Caller can
// modify if necessary. (Unlikely.)

CyclicWord* to_cyclic_word(FGWord const& word)
{
  CyclicWord *cw = NEW_STRUCT(CyclicWord);
  int i, n = word.length();

  // Create an empty word.
  cw->itsLength = n;
  cw->itsLetters = 0;
  cw->is_Dehn_relation = FALSE;
  cw->next = 0; 
  ...
  Letter *nl, *pl;
  for (i=0; i<n; ++i) {
    nl = NEW_STRUCT(Letter);
    nl->itsValue = word[i];

    // Install first letter, previous is self. 
    if (!i) {
      cw->itsLetters = nl; 
      pl = nl; 
    }
    
    // Link this letter to previous. 
    nl->prev = pl; 
    pl->next = nl;
    
    // New letter becomes previous.
    pl = nl; 
  }

  // Sort out first and last letter links. 
  if (n > 1) {
    nl->next = cw->itsLetters;
    cw->itsLetters->prev = nl;
  }
...
{
  MoebiusTransformation mt = MI(); 
  int g, i, n=word.length(); 
  for (i=0; i<n; i++) {
    g = word[i] > 0 ? word[i] : -word[i]; 
    if (g > group->itsNumGenerators || g==0) {
      printf("Non existent generator in word passed to word_to_Moebius\n"); 
      continue; 
    }
    if (word[i] > 0) 
      mt *= group->itsMoebiusTrs[g-1];
    else 
      mt *= inverse(group->itsMoebiusTrs[g-1]);
  }
  return mt; 
}

// If word_to_Moebius(group, word) maps a to b then 
// word_to_Moebius(group, word, conj) maps conj * a to conj * b. 

MoebiusTransformation word_to_Moebius(const GroupPresentation *group, const FGWord& word, const MoebiusTransformation& conj, int conj_by_inverse)
{
  if (conj_by_inverse)
    return inverse(conj) * word_to_Moebius(group, word) * conj; 
  return conj * word_to_Moebius(group, word) * inverse(conj);
}
...
void word_to_matrix(const GroupPresentation *group, const FGWord& word, O31_matrix& result)
{
  ...
  for (i=0; i<n; i++) {
    g = word[i] > 0 ? word[i] : -word[i]; 
    if (g > group->itsNumGenerators || g==0) {
      printf("Non existent generator in word passed to word_to_matrix\n"); 
      continue; 
    }
    if (word[i] > 0) 
      result *= group->itsMatrices[g-1]; 
    else {
      result *= inverse(group->itsMatrices[g-1]);
    }
  }
}

void word_to_matrix(const GroupPresentation *group, const FGWord& word, O31_matrix& result, O31_matrix const& conj, int conj_by_inverse)
{
  word_to_matrix(group, word, result); 
  if (!conj_by_inverse)
    result.conjugate_by(conj); 
  else 
    result.conjugate_by(inverse(conj)); 
}  
...
O31_matrix word_to_matrix(const GroupPresentation *group, const FGWord& word, O31_matrix const& conj, int conj_by_inverse)
{
  O31_matrix result; 
  word_to_matrix(group, word, result, conj, conj_by_inverse);
  return result; 
}

string string_form(const CyclicWord& cw)
{
  string s; 
  Letter *f = cw.itsLetters; 
  Letter *j; 

  if (!f) return s; 

  j = f; 
  s += char((j->itsValue > 0) ? ('a' + j->itsValue - 1) : ('A' - 1 - j->itsValue)); 

  for (j=j->next; j!=f; j=j->next)
    s += char((j->itsValue > 0) ? ('a' + j->itsValue - 1) : ('A' - 1 - j->itsValue)); 

  return s;
}

string string_form(const GroupPresentation* group)
{
  string s("<"); 
  ...
  const CyclicWord *cw = group->itsRelations; 
...
void print(const GroupPresentation* G, bool mats)
{
  ...
  for (i=0; i<n; i++) {
    cout << G->itsMoebiusTrs[i] << endl; 
  }
}

O31_matrix* fg_get_generators(GroupPresentation* G)
{
  return G->itsMatrices; 
}

MoebiusTransformation* fg_Moebius_generators(GroupPresentation* G)
{
  return G->itsMoebiusTrs; 
}

static void append_letter(CyclicWord* w, char c)
{
  Letter *L = NEW_STRUCT(Letter);
  L->itsValue = islower(c) ? (c-'a'+1) : ('A'-c-1); 
  ...
  if (!w->itsLetters) {
    ...
    w->itsLength++;
    INSERT_AFTER(L, w->itsLetters->prev);
  }
}

static CyclicWord* make_word(const char* wd)
{
  CyclicWord* w = NEW_STRUCT(CyclicWord);
  w->itsLength = 0;
  w->itsLetters= 0;
  w->is_Dehn_relation=FALSE;
  w->next = 0; 

  while (*wd) {
    if (isalpha(*wd)) append_letter(w, *wd);
    ++wd; 
  }
  return w; 
}

static void slist_append(CyclicWord *&sl, CyclicWord* w)
{
  if (!sl) {
    sl = w; 
    return; 
...
{
  char buf[BUFSIZE]; 
  int i = -1, n = 0, r = 0, nread; 
  O31_matrix *m, *mats;
  MoebiusTransformation *mt, *mtl; 
  char s1[2], s2[2], ps[2];
  bool psl=false; 

  while (i < n && fgets(buf, BUFSIZE, fp)) {

    // skip comments and empty lines
    if (buf[0]=='%' || sscanf(buf, "%1s", s1)==EOF) continue; 

    if (n==0) { // Expecting a number of generators
      if (sscanf(buf, "%d", &n)!=1) return 0; 
      if (n <= 0) return 0; 
      mats = NEW_ARRAY(n, O31_matrix); 
      mtl  = NEW_ARRAY(n, MoebiusTransformation); 
      i = 0; 
      continue; 
    }

    // Read row r of matrix i

    if (!psl) {
      m = &mats[i]; 
      nread = sscanf(buf, "%lf %lf %lf %lf", 
	...
	psl = true; 
	printf("Reading SL(2,C) matrices\n");
      } else {
	r++; 
	if (r==4) { 
	  mtl[i] = MoebiusTransformation(*m); 
	  r=0; 
	  ++i; 
	}
      }
    ...
    if (psl) {
      mt = &mtl[i]; 
      nread = sscanf(buf, "%lf %1s %lf i %lf %1s %lf i %1s", 
		     &(mt->matrix[r][0].real), s1, &(mt->matrix[r][0].imag),
		     &(mt->matrix[r][1].real), s2, &(mt->matrix[r][1].imag), 
		     ps);
      if (nread < 6) {
	printf("Error reading SL(2,C) matrices\n");
	return 0; 
      }
      if (s1[0]=='-') mt->matrix[r][0].imag = -mt->matrix[r][0].imag;
      if (s2[0]=='-') mt->matrix[r][1].imag = -mt->matrix[r][1].imag;
      r++;
      if (r==2) {
	mt->parity = (nread==7 && ps[0]=='*') ? 
	  orientation_reversing : orientation_preserving; 
	mats[i] = O31_matrix(*mt); 
	r = 0; 
	++i; 
      }
    }
  ...
  GroupPresentation* G = NEW_STRUCT(GroupPresentation); 

  G->itsNumGenerators = n;
  G->itsMatrices  = mats; 
  G->itsMoebiusTrs = mtl; 
  G->itsNumRelations = 0; 
  G->itsRelations = 0; 
  ...
  G->itsMeridians = 0; 
  G->itsLongitudes = 0;
  G->integer_fillings = FALSE;
  G->simplify_presentation = FALSE;
  G->fillings_may_affect_generators = FALSE;
  G->minimize_number_of_generators = FALSE;
  G->itsNumOriginalGenerators = 0; 
  G->itsOriginalGenerators = 0; 
  ...
  while (i < n && fgets(buf, BUFSIZE, fp)) {
    ...
    // skip empty lines
    if (sscanf(buf, "%1s", s1)==EOF) continue; 

    if (n==-1) { // Waiting for a keyword
      if (strncmp(buf, "% Relations", 11)==0) 
	doing_rels=true; 
      else if (strncmp(buf, "% Parabolics", 12)==0)
	doing_rels=false; 
      else {
	printf("Error while reading relations: %s", buf);
	printf("n = %d\n", n); 
	return 0; 
      }
      n=0;
    ...
    if (n==0) { // Expecting a number of relations
      if (sscanf(buf, "%d", &n)!=1 || n < 0) {
	printf("Illegal number of relations: %s", buf);
	return 0; 
      } 
      if (n==0) { // Go back to looking for a keyword. 
	if (!doing_rels) break; 
	n=-1; i=-2; continue; 
      }
      i = 0; 
    ...
    if (doing_rels) {
      w = make_word(buf);
      slist_append(G->itsRelations, w); 
      G->itsNumRelations++; 
    } else {
      dehn[0][0] = '\0';
      dehn[1][0] = '\0';
      sscanf(buf, "%s %s", dehn[0], dehn[1]);
      slist_append(G->itsMeridians, make_word(dehn[0]));
      slist_append(G->itsLongitudes, make_word(dehn[1]));
      G->itsNumCusps++;
    }
    ++i; 
    if (i==n && doing_rels) {
...
void save_presentation(FILE* fp, GroupPresentation* G, bool closed)
{
  int i, r, n_gens = fg_get_num_generators(G); 
  ...
  for (i=0; i<n_gens; ++i) {
    ...
    snappea_print_o31(G->itsMatrices[i], fp); 

#if 0
    mt=G->itsMoebiusTrs[i];
    for (r=0; r<2; ++r) {
      z = mt.matrix[r][0]; w = mt.matrix[r][1];
      fprintf(fp, "% 20.16f + % 20.16f i % 20.16f + % 20.16f i\n", 
  ...
  string mer, lon;
  fprintf(fp, "%% Parabolics\n%d\n", n_cusps);
  for (i=0; i<n_cusps; ++i) {
    mer = string(fg_meridian(G, i));
    lon = string(fg_longitude(G, i));
    fprintf(fp, "%s %s\n", mer.c_str(), lon.c_str()); 
  }
}
...
// These are needed by Dirichlet2D.

void init_name(CyclicWord **name)
{
  Letter *first,*second;
  first = NEW_STRUCT(Letter);
  second = NEW_STRUCT(Letter);
  first->itsValue = 25;
  second->itsValue = -25;
  first->next = first;
  first->prev = first;
  INSERT_BEFORE(second,first);
 *name = NEW_STRUCT(CyclicWord);
 (*name)->itsLength = 2;
 (*name)->itsLetters = second;
}
...
void simplify_word(CyclicWord *word)
{
  int   i;
  static Letter Z;
  Letter    *letter,*dead_letter;
  INSERT_BEFORE(&Z,word->itsLetters);
  Z.itsValue = 10000;
  for (  letter = word->itsLetters;
	 letter != Z.prev;
	letter = letter->next)
...
/*      my_free(letter->next);
	...
	letter = letter->prev;
	dead_letter = letter->next;
	if(dead_letter == word->itsLetters)
	  word->itsLetters = dead_letter->next;
	REMOVE_NODE(dead_letter);
	my_free(dead_letter);
...
void print_word(CyclicWord *name, FILE* out)
{
  ...
  for (i=0, letter = name->itsLetters;
       i < name->itsLength;
       i++, letter = letter->next) { 

    if (letter->itsValue < 0) {
      j = -1 * (letter->itsValue) - 1;
      fprintf(out, "%c",'A'+j);
    } else { 
      j = (letter->itsValue) - 1;
      fprintf(out, "%c",'a'+j);
...
{
  Letter* l = word->itsLetters; 
  int i; 

  printf("("); 
  for (i=0; i < word->itsLength; i++, l = l->next) {
    printf("%d", l->itsValue); 
    if (i < word->itsLength-1) printf(" "); 
  }
  printf(")"); 
}
#endif
