    ...
    WEPolyhedron    *polyhedron)
{
	...
	if (face->f_class == NULL)
	{
	    ...
	    face->f_class->parity = determinant(*face->group_element) > 0.0 ?
    ...
    WEPolyhedron    *polyhedron)
{
	...
	for (i = 0; i < 2; i++)
	{
	    ...
	    edge->preserves_direction[i]    = (unsigned char)-1;
	    edge->preserves_orientation[i]  = (unsigned char)-1;
	}
    }
}
    ...
    WEFace  *face)
{
    ...
    face_vertices = NEW_ARRAY(face->num_sides, O31_vector);
    mate_vertices = NEW_ARRAY(face->num_sides, O31_vector);
    ...
    traverse_clockwise = determinant(*face->group_element) > 0.0;
	...
	mate_vertices[count] = *face->group_element * O31_vector(vertex->x); 
	for (i = 1; i < 4; i++)
     ...
     */
    my_free_array(face_vertices);
    my_free_array(mate_vertices);
}
    ...
    WEPolyhedron    *polyhedron)
{
	...
	if (face->mate == face)
	{
	    ...
	    tr = trace(*face->group_element);

	    if (fabs(fmod(fabs(tr) + 0.5, 1.0) - 0.5) > TRACE_ERROR_EPSILON)
		uFatalError("subdivide_faces_where_necessary", "Dirichlet_extras");

	    switch ((int) floor(tr + 0.5))
	    {
		case -2:
    ...
    WEPolyhedron    *polyhedron)
{
     ...
     */
    for (i = 0; i < 4; i++)
	fixed_point[i] = (i == 0 ? rONE : rZERO) + (*face->group_element)(i,0);
    central_vertex->x = fixed_point/fixed_point[0]; 
	...
	*new_face[i]->group_element = *face->group_element;
	new_face[i]->word = face->word; 
    ...
    WEPolyhedron    *polyhedron)
{
	 ...
	 */
	angle_between_normals = acos(normal[left] * normal[right]);
    ...
    WEVertex    *vertex)
{
    ...
    norm_squared = vertex->x.norm_squared();

    if (norm_squared < - IDEAL_EPSILON)
    {
	vertex->dist    = acosh( safe_sqrt( -1.0 / norm_squared ) );
	vertex->ideal   = FALSE;
    }
    else
    {
...
const O31_vector basepoint(1.0, 0.0, 0.0, 0.0);
    ...
    WEEdge  *edge)
{
    ...
    if (u_coord < p0_coord)
    {
	edge->closest_point_on_edge = p[0];
	edge->dist_edge_to_origin = edge->v[tail]->dist;
    }
    else if (u_coord > p1_coord)
    {
	edge->closest_point_on_edge = p[1];
	edge->dist_edge_to_origin = edge->v[tip]->dist;
    }
    else
    {
	edge->closest_point_on_edge = edge->closest_point_on_line;
    ...
    WEPolyhedron    *polyhedron)
{
    ...
    WEFace      *face;
    int         i;
    O31_vector   the_image,
		the_midpoint;

    static O31_vector the_origin(1.0, 0.0, 0.0, 0.0);
	...
	face->dist          = rHALF * acosh((*face->group_element)(0,0));
	 ...
	 */
	the_midpoint = rHALF * (the_origin + the_image); 
	 ...
	 */
	face->closest_point = the_midpoint/the_midpoint[0]; 
    }
}
    ...
    WEEdge  *edge)
{
	...
	edge->length = acosh(
	    -edge->v[tail]->x * edge->v[tip]->x
	    /
	    (
		sqrt(-edge->v[tail]->x * edge->v[tail]->x)
	      * sqrt(-edge->v[tip ]->x * edge->v[tip ]->x)
	    ));
}
    ...
    WEPolyhedron    *polyhedron)
{
     ...
     */
    O31_vector   a,      /*  at vertex   = (4) above     */
		b,      /*  on edge     = (3) above     */
		c,      /*  on face     = (2) above     */
		d;      /*  at origin   = (1) above     */
    GL4RMatrix  abcd;

    static O31_vector    origin(1.0, 0.0, 0.0, 0.0);
    ...
    for (edge = polyhedron->edge_list_begin.next;
	 ...
	 edge = edge->next)
    {
	b = edge->closest_point_on_line;

	for (i = 0; i < 2; i++)     /*  i = left, right */
	{
	    c = edge->f[i]->closest_point;
    ...
    WEPolyhedron    *polyhedron)
{
    ...
    polyhedron->inradius = 0.5 * acosh(min_value);
}
    ...
    WEPolyhedron    *polyhedron)
{
     ...
     */
    polyhedron->outradius = acosh(rONE/sqrt(rONE - max_projective_distance));
}
    ...
    WEPolyhedron    *polyhedron)
{
    ...
    for (face = polyhedron->face_list_begin.next;
	 ...
	 face = face->next)
    {
	the_deviation = face->group_element->deviation();
