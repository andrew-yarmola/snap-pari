...
/*
 *  Added by oag. Keep track of corner positions whenever possible
 *  in one_to_four and two_to_three, called by canonize_part_2.c
 */

#define KEEP_CORNERS 1
    ...
    Triangulation   *manifold)
{
    ...
    if (original_solution_type[complete] != not_attempted)
    {
    ...
    Triangulation   *manifold)
{
     ...
     *  recompute the hyperbolic structure, and recompute the CS_fudge.
     *
     *  97/2/3  Let basic_simplification() know whether it should
     *  recompute the hyperbolic structure at the end.
     */
    if (original_solution_type[complete] != not_attempted)
    {
	manifold->solution_type[complete]   = original_solution_type[complete];
...
// For each glued face of a tetrahedron there are three
// pairs of glued triangles in the cusp. We set up the
// peripheral curves of the partner's triangles so that
// they sum to zero at the common faces. 

static void set_partner_curves(Tetrahedron* tet, FaceIndex k)
{
  Tetrahedron* ptet = tet->neighbor[k]; 
  Permutation  perm = tet->gluing  [k]; 
  // want h = 0 if orientation preserving. 
  int h = (parity[perm]==orientation_reversing);

  FaceIndex pk = EVALUATE(perm, k); 
  VertexIndex v, pv; 
  int c; 
  for (v=0; v<4; v++) {
    if (v==k) continue; 
    pv = EVALUATE(perm, v); 
    for (c=0; c<2; c++) {
      ptet->curve[c][0][pv][pk] = -tet->curve[c][  h][v][k];
      ptet->curve[c][1][pv][pk] = -tet->curve[c][1-h][v][k];
    }
  }
}

static void set_third_side_curves(Tetrahedron* tet, VertexIndex v, FaceIndex k)
{
  EdgeIndex e = edge_between_vertices[v][k];
  FaceIndex f1= one_face_at_edge[e];
  FaceIndex f2= other_face_at_edge[e];
  int c, h; 

  for (c=0; c<2; c++) {
    for (h=0; h<2; h++) {
      tet->curve[c][h][v][k] = 
	-(tet->curve[c][h][v][f1] + tet->curve[c][h][v][f2]);
    }
  }
}

static void set_other_side_curves(Tetrahedron* tet, VertexIndex v, FaceIndex k)
{
  EdgeIndex e = edge_between_vertices[v][k];
  FaceIndex f1= one_face_at_edge[e];
  FaceIndex f2= other_face_at_edge[e];
  ...
  for (c=0; c<2; c++) {
    for (h=0; h<2; h++) {
      x = tet->curve[c][h][v][k]; 
      y = x/2; 
      x -= y; // divide as evenly as possible between x and y. 
      tet->curve[c][h][v][f1] = -x;
      tet->curve[c][h][v][f2] = -y; 
    }
  }
}

static void setup_edge_class(EdgeClass* edge, int maxord)
{
  // Walk around the edge. 
  Orientation relative_orientation = right_handed; 
  EdgeIndex e; 

  EdgeIterator it0(edge), it;
  for (it = it0; it.index < maxord; it++) {

    if (it.index && it==it0) break; // finished when we get back to start

    e = it.edge_index(); 

    it.tet->edge_class[e] = edge; 
    it.tet->edge_orientation[e] = relative_orientation; 

    if (parity[it.front_gluing()] == orientation_reversing)
      relative_orientation = !relative_orientation; 

  }

  if (it != it0 || relative_orientation != right_handed)
    uFatalError("setup_edge_class", "simplify_triangulation");

  edge->order = it.index;
  setup_edge_direction(edge);
}

static void print_angles_at_edge(EdgeClass *edge)
{    
  Complex asum(Zero), z; 
  cout << "edge has order " << edge->order << endl; 
  EdgeIterator it(edge), it0;
  it0 = it; 
  int ep; 
  for (; it.index < edge->order; it++) {
    ep = it.edge3(); 
    z = it.tet->shape[filled]->cwl[ultimate][ep].log;
    cout << "angle " << z << endl;
    asum += z; 
  }
  cout << "angle sum = " << asum << endl; 
  if (it != it0) {
    cout << "order of the edge is wrong!!\n";
  }
}

static bool validate_faces(EdgeClass *edge, int face[2])
{
  int i; 
  for (i=0; i<2; i++) face[i] %= edge->order; 
  // Check not both the same face. 
  if (face[0] == face[1]) return false; 
  // Put them in increasing order. 
  if (face[0] >  face[1]) std::swap(face[0],face[1]);
  return true; 
}

/* 
 *  We check that face[0],face[1] are distinct faces at edge.
 *  Assuming so, we slice open the triangulation there to form a 
 *  pocket with four triangles into which we insert two cancelling 
 *  tetrahedra. If we regard the two original faces as forming a
 *  square with diagonal, we are inserting a new pair of faces 
 *  which make up the same square but with the other diagonal. 
 * 
 *  Important: after calling this function we must call 
 *  tidy_peripheral_curves(manifold); 
 */

FuncResult uncancel_tetrahedra(EdgeClass *edge, int face[2], EdgeClass **new_e, int* num_tetrahedra_ptr)
{
  if (!validate_faces(edge, face)) return true; 

  int i, j, k, v, ep, c, s; 

  int found=0; // Count faces found. 

  // We want to add up the total log angles on each
  // side of the two faces, so we need to keep track
  // of which side we are on. 
  int side=0; 
  Complex log_angle_sum[2][2][2]; // [side][filled][ultimate]
  for (i=0; i<2; i++) 
    for (j=0; j<2; j++) 
      for (k=0; k<2; k++) 
	log_angle_sum[i][j][k] = Zero; 

  // Now work around the edge.
  // We will set B up to be the boundary of the cut-open triangulation. 
  EdgeIterator it(edge), B[4];
  for (; it.index < edge->order; ++it) {

    // See if we've come to either of our faces. 
    for (j=0; j<2; j++) {
      if (face[j]==it.index) {

	// Save the information about the face and its partner. 
	B[2*j+1] = it; 
	B[2*j]   = it.partner(); 

	// cout << "found face " << face[j] << endl;

	side = 1-side; // Switch sides each time we pass a face.
	found++; 
      }
    }

    // Keep track of log angle sum on this side. 
    if (it.tet->shape[ultimate] != NULL) {
      ep = it.edge3(); 
      for (j=0; j<2; j++) { // complete/filled
	for (k=0; k<2; k++) { // penultimate/ultimate
	  log_angle_sum[side][j][k] += it.tet->shape[j]->cwl[k][ep].log;
	}
      }
    }
  }
  
  // Check that we found both faces. 
  if (found != 2) return func_failed;

  // And that they are truly distinct.
  for (i=0; i<2; i++) {
    if (B[i].tet == B[2].tet && B[i].face() == B[2].face()) return func_failed;
  }

  // Make the new tetrahedra. 
  Tetrahedron* new_tet[2]; 
  for (j=0; j<2; j++) {
    new_tet[j] = NEW_STRUCT(Tetrahedron);
    initialize_tetrahedron(new_tet[j]);
  }

  Permutation perm = CREATE_PERMUTATION(0,B[0].V[0],1,B[0].V[1],2,B[0].V[2],3,B[0].V[3]);
  int koff = (parity[perm] == orientation_reversing) ? 1 : 0; 

  // Do gluings at the B[i]. 
  for (i=0; i<4; i++) { 

    j = (i+1)/2%2;    // glues to new_tet[j], j = 0,1,1,0
    k = (i+koff)%2+2; // glues to face k,     k = 2,3,2,3 (koff==0)
    v = 5-k;     // V[3] glues to vertex v

    new_tet[j]->neighbor[k] = B[i].tet;
    new_tet[j]->gluing  [k] = 
      CREATE_PERMUTATION(0,B[i].V[0],1,B[i].V[1],v,B[i].V[3],k,B[i].V[2]);
    set_inverse_neighbor_and_gluing(new_tet[j], k);

    // Cusp fields (equatorial)
    new_tet[j]->cusp[v] = B[i].tet->cusp[B[i].V[3]];
  }

  // Do 'internal' gluings
  Permutation p0132 = CREATE_PERMUTATION(0,0,1,1,2,3,3,2);
  for (j=0; j<2; j++) {
    for (k=0; k<2; k++) {
      new_tet[j]->neighbor[k] = new_tet[1-j]; 
      new_tet[j]->gluing  [k] = p0132; 

      // Cusp fields (polar)
      v = 1-k; 
      new_tet[j]->cusp[v]= B[1].tet->cusp[B[1].V[v]];
    }
  }

  // Set peripheral curves (external faces)
  for (i=0; i<4; i++) {
    set_partner_curves(B[i].tet, B[i].V[2]); 
  }
  // Set peripheral curves (internal faces of new_tet[0]). 
  for (v=0; v<2; v++) {
    set_third_side_curves(new_tet[0],  v,1-v);
    set_other_side_curves(new_tet[0],v+2,3-v);
  }
  // Set peripheral curves (internals of new_tet[1]). 
  for (k=0; k<2; k++) {
    set_partner_curves(new_tet[0], k); 
  }

  // NOW WORK ON THE EDGES

  // The old edge keeps tets B[3] - B[0] 
  B[3].set_edge_base(edge); 
  setup_edge_class(edge, edge->order+12);

  // The new vertical edge gets tets B[1] - B[2]
  EdgeClass *v_edge = NEW_STRUCT(EdgeClass); 
  initialize_edge_class(v_edge); 
  B[1].set_edge_base(v_edge);
  setup_edge_class(v_edge, edge->order+12);
  if (new_e) *new_e = v_edge; 

  // The new horizontal edge
  EdgeClass *h_edge = NEW_STRUCT(EdgeClass); 
  initialize_edge_class(h_edge); 
  h_edge->incident_tet = new_tet[0]; 
  h_edge->incident_edge_index= edge_between_faces[0][1]; 
  setup_edge_class(h_edge, 2);

  // Store them after edge in order edge,v_edge,h_edge
  INSERT_BEFORE(h_edge, edge->next);
  INSERT_BEFORE(v_edge, h_edge); 

  // Sort out the other 4 edges
  EdgeClass *oe;
  for (i=1; i<4; i+=2) {
    for (v=0; v<2; v++) {
      oe = B[i].tet->edge_class[edge_between_vertices[B[i].V[v]][B[i].V[3]]];
      setup_edge_class(oe, oe->order+12); 
    }
  }

  // COMPUTE THE NEW SHAPES 

  // log_angle_sum[0] contains sum for tets B[3] - B[0]
  // log_angle_sum[1] contains sum for tets B[1] - B[2]

  int e01 = edge3_between_faces[0][1], e;
  if (B[0].tet->shape[complete] != NULL) {

#if 0
    cout << "Sum around vertical edge: "; 
    cout << (log_angle_sum[0][filled][ultimate] + log_angle_sum[1][filled][ultimate]) 
	 << endl;
#endif 

    for (i=0; i<2; i++) { // complete/filled
      for (j=0; j<2; j++) {
	new_tet[j]->shape[i] = NEW_STRUCT(TetShape);
      }
    }

      
    // Fill in the shape of new_tet[0]. 
    for (i=0; i<2; i++) {
      for (k=0; k<2; k++) { // penultimate/ultimate
	new_tet[0]->shape[i]->cwl[k][e01].rect = 
	  complex_exp(-log_angle_sum[0][i][k]);
      }
    }
    compute_remaining_angles(new_tet[0], e01);


    // Set up new_tet[1] to cancel with it exactly.
    for (i=0; i<2; i++) {
      for (k=0; k<2; k++) {
	// Make sure angle sum is 2*Pi around e01. 
	new_tet[0]->shape[i]->cwl[k][e01].log = 
	  TwoPiI - log_angle_sum[0][i][k];

	// Want angle sum 0 around outer edges, 2*Pi around
	// the vertical and horizontal edges. 
	for (e=0; e<3; e++) {
	  new_tet[1]->shape[i]->cwl[k][e].rect = 
	    One/new_tet[0]->shape[i]->cwl[k][(3-e)%3].rect;
	  new_tet[1]->shape[i]->cwl[k][e].log = 
	    -new_tet[0]->shape[i]->cwl[k][(3-e)%3].log;
	  if (e == 0) { // == e01, the vertical/horizontal pair
	    new_tet[1]->shape[i]->cwl[k][e].log += TwoPiI;
	  }
	}
      }
    }
#if 0
    cout << "Original edge\n";
    print_angles_at_edge(edge); 
    cout << "New vertical edge\n";
    print_angles_at_edge(v_edge); 
    cout << "Horizontal edge\n";
    print_angles_at_edge(h_edge); 
#endif
  }

  for (j = 0; j < 2; j++)
    INSERT_BEFORE(new_tet[j], edge->incident_tet);

  *num_tetrahedra_ptr += 2;

  // If the manifold was orientable, keep it so. 
  for (j=0; j<2; j++) {
    for (k=2; k<4; k++) {
      if (parity[new_tet[j]->gluing[k]] == orientation_reversing) {
	cout << "oh no, orietation has been trashed!\n"; 
      }
    }
    // for (i=0; i<2; i++) reverse_orientation(new_tet[i]); 
  }
...
#if KEEP_CORNERS
	/*
	 *  If this is happening as part of canonize_part_2 we want
	 *  to keep track of the positions of all ideal vertices. 
	 *  Since, for step 2, one tetrahedron spans two cells which might not
	 *  be positioned so as to have a directly matching face we have 
	 *  a slight problem: vertices 2 and 3 each have two possible values
	 *  the value from the cell centered at vertex 0 and the value from
	 *  the cell centered at vertex 1. We need to record all of these
	 *  four values so here is how we do it. 
	 *  vertex 2 in cell 0 is stored in corner[2], 
	 *  vertex 3 in cell 1 is stored in corner[3],
	 *  vertex 3 in cell 0 is stored in corner[0] (if vertex 0 is finite). 
	 *  vertex 2 in cell 1 is stored in corner[1] (if vertex 1 is finite),
	 *  Thus to get the position of vertex k (= 2 or 3), of cell j
	 *  use new_tet[i]->corner[(k==j+2) ? k : j].
	 */
	for (i = 0; i < 3; i++) {
	  for (j = 0; j < 2; j++) {
	    new_tet[i]->corner[j+2] = tet[1-j]->corner[v[1-j][(i+j+1)%3]];
	    if (tet[1-j]->cusp[v[1-j][3]]->is_finite)
	      new_tet[i]->corner[j] = tet[1-j]->corner[v[1-j][(i+2-j)%3]];
	    else
	      new_tet[i]->corner[j] = tet[1-j]->corner[v[1-j][3]];
	  }
	}
    ...
    int         new_cusp_index)
{
	...
	for (i = 0; i < 4; i++)
	{
	    ...
	    /* 
	     *  Added by oag. 
	     *  Keep corner information for ideal vertices.
	     */
            for (j = 0; j < 4; j++) { /* each vertex */
	        if (i==j) continue;
	        new_tet[i]->corner[j] = tet->corner[j];
	    }
#endif
	}
    }
