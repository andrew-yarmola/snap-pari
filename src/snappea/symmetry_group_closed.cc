    ...
    Triangulation   *manifold)
{
    ...
    static int      num_precisions = 5;
    static double   precision[5] = {1e-8, 1e-6, 1e-10, 1e-4, 1e-12};

    for (i = 0; i < num_precisions; i++)
    {
	polyhedron = Dirichlet(manifold, precision[i], TRUE, TRUE, FALSE);
	if (polyhedron != NULL)
	    return polyhedron;
    }
    ...
    int             *num_lengths)
{
    ...
    static double   max_tiling_radius = 4.0;
    static int      enough_lengths = 3;
...
static double rigor_radius(
    ...
    double  cutoff_length)
{
    return 2*acosh(cosh(spine_radius)*cosh(cutoff_length/2));
}
    ...
    Triangulation   **manifold)
{
    ...
    static Boolean  fill_cusp[2] = {TRUE, FALSE};
