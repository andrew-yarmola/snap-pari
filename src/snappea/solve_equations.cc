    ...
    Complex *solution)
{
	...
	for (r = c + 1; r < num_rows; r++)
	{
	    ...
	    if (sign(factor_real) || sign(factor_imag))
	    {
		...
		{
		    if (sign(row_c->real) || sign(row_c->imag))
		    {
			row_r->real +=
    ...
    int     num_columns)
{
	...
	if (max_modulus == 0.0)     /* In the old snappea, max_modulus  */
	    return Zero;     /* was was never below 1e-100, even */
				    /* in degenerate cases.             */
