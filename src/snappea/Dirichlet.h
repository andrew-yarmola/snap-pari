...
/*
 *  The MatrixPair data structure stores an O31_matrix and its inverse.
 */
...
class MatrixPair
{
  ...
  public:
    ...
    MatrixPair(); 
    MatrixPair(O31_matrix const& m, int gnum); 
    MatrixPair(O31_matrix const& m, const FGWord& w); 
    MatrixPair(O31_matrix const& a, const FGWord& w, O31_matrix const& b);

    Boolean contains(O31_matrix const& m) const; 
    double height() const { return _height; }
    const O31_matrix& M(int i) const { return m[i]; }
    FGWord word(int i) const { return i ? inverse(_word) : _word; }
    Boolean is_identity() const;
    Boolean fixes_basepoint() const;
    FuncResult set_objective_function(ObjectiveFunction objective_function) const;
    void o31_conjugate(O31_matrix const& t); 

    void set_M(O31_matrix const& new_m); 
    void set_M(O31_matrix const& new_m, const FGWord& new_w);
    void precise_matrix_pair();
};
...
extern void         maximize_injectivity_radius(MatrixPairList *gen_list, Boolean *basepoint_moved, Boolean interactive, O31_matrix& conjugacy);
extern void         conjugate_matrices(MatrixPairList *gen_list, double displacement[3], O31_matrix & t2);
extern void         free_matrix_pairs(MatrixPairList *gen_list);

extern void         precise_o31_product(O31_matrix const& a, O31_matrix const& b, O31_matrix& product);
...
extern Boolean singular_set_failure; 
